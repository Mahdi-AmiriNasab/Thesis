...
{183,   173,    163,    153,144,135,120,128,113,
 ...
 36.70, 34.97, 33.33,31.77, 30.25, 28.82, 27.45, 26.16, 24.94, 23.77, 22.67,
...
uint16_t counterCHM=0;


uint16_t Flag_Fast_Charge_Counter=0,Flag_Fast_Charge=0;
...
uint16_t CANBMBCounter[8],bmbReadyCounter[8];
uint16_t BMBFailedCounter=0;
uint16_t flag100ms=0;
uint16_t startAlgorithmFlag = 0;

double test,test1,test2;

uint16_t max=0,max1=0;
uint16_t totalBalancerCount=0,totalBalancerCountTemp=0;
double cellCurrent;

uint16_t ADC[4];
double test4;

uint16_t timerbmb=0,BMBQualityCounter=0;

uint16_t calculationFlag = 0;


float inputVoltage, currentLow, currentHigh, packVoltage,packVoltageX, currentLowFilter, currentHighFilter;


uint16_t CurrentCalFlag=0;

uint16_t BMBCounter1,BMBCounter2,BMBCounter3,BMBCounter4,BMBCounter5,BMBCounter6,BMBCounter7,BMBCounter8;


uint16_t bmbReadyFlag[8];

uint16_t estimationCount=0;

uint16_t CANQualityFlag, lowSpeedFlag=0;

uint16_t Flag_test3;
uint16_t Flag_test3_CAN_Fault;
double packCurrent;
uint16_t flagStartMeasure=0;

double currentIntegral=0, SOC_columb=0;

uint16_t testCounter=0,timer=0;
uint16_t testbmbcounter=0;

uint16_t NegContFlag,PosContFlag,PrechargeContFlag;

uint16_t ShortCircuitFault = 0;

uint16_t estimationFlag = 0;

uint16_t FlagStartEstimation=0;
uint16_t flagEpwm1Stop=0, flagEpwm2Stop=0;

double OCV_table = 0;
uint16_t OCV_GetFlag =0,FlagStartOCV=0;
...
*/
        ...
        if(CurrentCalFlag == 1){
            ...
            CAN_U.test1 = (uint16_t)((double)currentLow*10+5000.0) & 0xFF;
            CAN_U.test2 = ((uint16_t)((double)currentLow*10+5000.0) >> 8) & 0xFF;
            CAN_U.test3 = (uint16_t)((double)currentHigh*10+5000.0) & 0xFF;
            CAN_U.test4 = ((uint16_t)((double)currentHigh*10+5000.0) >> 8) & 0xFF;
